...
class TextCNNRNN(object):
	def __init__(self, embedding_mat, sequence_length,num_classes, filter_sizes,dictionnaire):
		...
		##### 1
		self.string_to_manipulate = tf.placeholder(tf.string , [None])
		self.dictionnaire = tf.constant(dictionnaire)
		#### 2
		self.input_x = tf.placeholder(tf.int32, [None,sequence_length], name='to_input_x')#done
		### 1
		self.input_x = tf.map_fn(lambda x: tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(tf.regex_replace(x,"\s{2,}", " "),"\?", " \? "),"\)", " \) "),"\(", " \( "),"!", " ! "),",", " , "
			),"\'ll", " \'ll"),"\'d", " \'d"),"\'re", " \'re"),"n\'t", " n\'t"),"\'ve", " \'ve"),"\'s", " \'s")," : ", ":"),"[^\u0627-\u064aA-Za-z0-9:(),!?\'\`]", " "),self.string_to_manipulate)
		self.input_x = tf.map_fn(lambda x:tf.string_split([x],delimiter=' ',skip_empty=True).values,self.string_to_manipulate)
		self.input_x = tf.map_fn(lambda x1:tf.map_fn(lambda x: tf.cond(tf.equal(tf.size(tf.where(tf.equal(self.dictionnaire, [x]))[::,-1]),0),lambda :tf.constant(0,dtype=tf.int32),lambda :tf.cast(tf.where(tf.equal(self.dictionnaire, [x]))[::,-1][0],dtype=tf.int32)),x1,dtype=tf.int32),self.input_x,dtype=tf.int32)
		self.input_x = tf.map_fn(lambda solv:tf.concat([ solv , tf.zeros([tf.cond(tf.less(sequence_length-tf.size(solv), 0),lambda : 0 , lambda  :sequence_length-tf.size(solv))],dtype=tf.int32 ) ],0 ),self.input_x)
		self.input_x = tf.map_fn(lambda solv:tf.cond( tf.less(sequence_length, tf.size( solv )),lambda : tf.slice( solv ,[0],[sequence_length]), lambda  :solv ),self.input_x)
		### 2
		...
		self.sizeOfInputX = tf.size(self.input_x)
		self.batch_size = tf.cast(self.sizeOfInputX / sequence_length,dtype=tf.int32)
		self.pad = tf.zeros([self.batch_size,1,self.embedding_dim,1])#sho
		self.real_len =tf.map_fn(lambda x: tf.cast(tf.ceil(tf.multiply(tf.cast(tf.argmin(tf.concat([x ,[0]],0)),dtype=tf.float32),1.0)/4),dtype=tf.int32),self.input_x)
		...
		pooled_concat = []
		reduced = np.int32(np.ceil((sequence_length) * 1.0 / self.max_pool_size))
