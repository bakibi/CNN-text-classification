...
def train_cnn_rnn():
	...
	x_, y_, vocabulary, vocabulary_inv, df, labels = data_helperA.load_data(input_file)
	...
	word_embeddings = data_helperA.load_embeddings(vocabulary)
	...
	with graph.as_default():
		...
		with sess.as_default():
			print('------------------------------++ ','begin initializing our cnn object')
			...
			print('------------------------------++ ','End initializing our cnn object')
			...
			def train_step(x_batch, y_batch):
				...
				feed_dict = {
					cnn_rnn.string_to_manipulate: x_batch,
			...
			print('------------------------------++ ','sess was initilaized with initializing all the variables')
			# Training starts here
			train_batches = data_helperA.batch_iter(list(zip(x_train, y_train)), params['batch_size'], params['num_epochs'])
			...
			for train_batch in train_batches:
				...
				if current_step % params['evaluate_every'] == 0:
					dev_batches = data_helperA.batch_iter(list(zip(x_dev, y_dev)), params['batch_size'], 1)
			...
			to_input_x = tf.saved_model.utils.build_tensor_info(cnn_rnn.string_to_manipulate)
			to_predictions = tf.saved_model.utils.build_tensor_info(cnn_rnn.predictions)
			...
			test_batches = data_helperA.batch_iter(list(zip(x_test, y_test)), params['batch_size'], 1, shuffle=False)
